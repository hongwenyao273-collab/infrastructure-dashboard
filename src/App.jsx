...
/*
  ...
  1. Project selection is separated from Add New Project.
  2. Selected project information can be viewed and edited.
  3. Stakeholder weights and project selection are linked.
  4. Live Project Ranking shows weighted score contribution, not fixed raw score bars.
  5. Scenario table clearly labels Rank and Score instead of using unclear bracket notation.
*/

const STORAGE_KEY = "engg3112_sensitivity_dashboard_projects_v4";
...
function getScenarioList(activeWeights) {
  ...
  return [
    { id: "current", name: "Current / Baseline", shortName: "Current", weights: balanced },
...
function classifyPriority(weights) {
  ...
  const entries = [
    ...
    ["Environmental Priority", normalised.environmental, "environmental performance has the largest weighting"],
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl bg-white p-3 shadow-sm">
        ...
        <p className="mb-3 text-sm text-slate-500">
          These weights are used when this project is selected as the baseline. They are normalised to 100%.
        </p>
        <div className="grid grid-cols-3 gap-3">
...
function ProjectSelection({ projects, selectedProjectId, onSelectProject, onDeleteProject, onResetProjects, onSetEqualWeights, onUseBaseline }) {
  return (
    <Card>
      <CardContent>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={onUseBaseline}><RotateCcw className="mr-2 inline h-4 w-4" /> Baseline</Button>
          <Button variant="outline" onClick={onSetEqualWeights}><Equal className="mr-2 inline h-4 w-4" /> Equal</Button>
        </div>

        <div className="mt-2 grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={onResetProjects}><RotateCcw className="mr-2 inline h-4 w-4" /> Reset Data</Button>
...
function SelectedProjectEditor({ selectedProject, onSaveProject }) {
    ...
    <Card>
      <CardContent>
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold">Selected Project Information</h2>
            <p className="mt-1 text-sm text-slate-500">View and edit the selected project scores and baseline weights.</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-600">
        Adjust stakeholder or policy weights. The dashboard normalises the raw total to 100 before calculating rankings.
      </div>

      <div className="space-y-4">
        <div>
          <div className="mb-1 flex justify-between text-sm"><span>Financial weight</span><strong>{weights.financial}%</strong></div>
          <input type="range" min="0" max="100" value={weights.financial} onChange={(event) => setWeights((prev) => ({ ...prev, financial: Number(event.target.value) }))} className="w-full accent-blue-600" />
        </div>
        <div>
          <div className="mb-1 flex justify-between text-sm"><span>Environmental weight</span><strong>{weights.environmental}%</strong></div>
          <input type="range" min="0" max="100" value={weights.environmental} onChange={(event) => setWeights((prev) => ({ ...prev, environmental: Number(event.target.value) }))} className="w-full accent-emerald-600" />
        </div>
        <div>
          <div className="mb-1 flex justify-between text-sm"><span>Social weight</span><strong>{weights.social}%</strong></div>
          <input type="range" min="0" max="100" value={weights.social} onChange={(event) => setWeights((prev) => ({ ...prev, social: Number(event.target.value) }))} className="w-full accent-purple-600" />
        </div>
      </div>

      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-2xl bg-blue-50 p-3 text-center text-blue-700"><p className="text-xs">F normalised</p><p className="text-2xl font-bold">{normalised.financial}%</p></div>
        <div className="rounded-2xl bg-emerald-50 p-3 text-center text-emerald-700"><p className="text-xs">E normalised</p><p className="text-2xl font-bold">{normalised.environmental}%</p></div>
        <div className="rounded-2xl bg-purple-50 p-3 text-center text-purple-700"><p className="text-xs">S normalised</p><p className="text-2xl font-bold">{normalised.social}%</p></div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-2xl bg-white/70 p-3">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Weighted Score Contribution</p>
        <div className="space-y-3 text-xs">
          <div>
            <div className="mb-1 flex justify-between"><span>Financial: {project.financialScore} × {activeWeights.financial}%</span><strong>{contributions.financial}</strong></div>
            <ScoreBar value={contributions.financial} tone="blue" max={100} />
          </div>
          <div>
            <div className="mb-1 flex justify-between"><span>Environmental: {project.environmentalScore} × {activeWeights.environmental}%</span><strong>{contributions.environmental}</strong></div>
            <ScoreBar value={contributions.environmental} tone="green" max={100} />
          </div>
          <div>
            <div className="mb-1 flex justify-between"><span>Social: {project.socialScore} × {activeWeights.social}%</span><strong>{contributions.social}</strong></div>
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 p-3 text-sm text-slate-600">
              <strong>How to read this table:</strong> a lower rank is better. For example, <span className="font-semibold">Rank 1, Score 66.8/100</span> means the project is ranked first under that scenario and receives a weighted score of 66.8 out of 100.
            </div>
          </div>
        </div>
...
export default function MultiCriteriaDashboard() {
  ...
  const [activeWeights, setActiveWeights] = useState(projects[0]?.initialWeights || { financial: 33, environmental: 33, social: 34 });
  ...
  function handleSaveProject(updatedProject) {
    ...
    setActiveWeights(updatedProject.initialWeights);
  }
  ...
  function handleDeleteProject(projectId = selectedProjectId) {
    ...
    setActiveWeights(nextSelected?.initialWeights || { financial: 33, environmental: 33, social: 34 });
  }
  ...
  function resetProjects() {
    ...
    setActiveWeights(defaultProjects[0].initialWeights);
  }
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mx-auto max-w-7xl space-y-5">
        <div className="flex flex-col justify-between gap-5 rounded-3xl bg-slate-950 p-6 text-white shadow-lg lg:flex-row lg:items-center">
          ...
          <div className="grid grid-cols-3 gap-3 rounded-2xl bg-white/10 p-4 text-center">
            ...
            <div><p className="text-xs text-slate-300">Active Weights</p><p className="text-lg font-bold">{normalisedWeights.financial}/{normalisedWeights.environmental}/{normalisedWeights.social}</p></div>
          </div>
        </div>
          ...
          <div className="space-y-5 lg:col-span-4">
            ...
            <Card>
              <CardContent>
                ...
                {showAddProject ? <AddProjectForm onAdd={handleAddProject} /> : <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">Use this section only when creating a new infrastructure option. Project selection and editing are handled separately above and beside this module.</p>}
              </CardContent>
            </Card>
          </div>

          <div className="lg:col-span-4">
            <SelectedProjectEditor selectedProject={selectedProject} onSaveProject={handleSaveProject} />
          </div>
